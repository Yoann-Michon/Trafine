...
import { formatDistance, formatDuration, getETA } from '@/lib/tomtom';
...
export default function RouteControls({ navigation, userLocation, onRouteCalculated }: RouteControlsProps) {
  ...
  useEffect(() => {
    ...
    if (handleElement) {
      handleElement.addEventListener('touchstart', (e) => handleTouchStart(e as TouchEvent));
    }
    
    return () => {
      if (handleElement) {
        handleElement.removeEventListener('touchstart', handleTouchStart as EventListener);
      }
      document.removeEventListener('touchmove', handleTouchMove);
      document.removeEventListener('touchend', handleTouchEnd);
    };
                ...
                <div className="mt-3 p-4 bg-primary bg-opacity-5 rounded-lg">
                  <div className="flex justify-between items-center">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-neutral-900">
                        {getETA(navigation.routeData.routes[0].summary.travelTimeInSeconds)}
                      </p>
                    </div>
                  </div>
                </div>
